...
export function dataH(filtro,bool) {
  ...
  for (var i = 0; i < items.eventos.length; i++) {
    if(filtro == "todos" || filtro == items.eventos[i].tipo || filtro == items.eventos[i].semestre){
      ...
      var mostrar;
      if(parseInt(año_fin) > año) mostrar = true;
      else 
        if(parseInt(año_fin) == año && parseInt(mes_fin) > mes) mostrar = true;
        else 
          if(parseInt(año_fin) == año && parseInt(mes_fin) == mes && parseInt(dia_fin) >= dia) mostrar = true;
          else mostrar = false;
